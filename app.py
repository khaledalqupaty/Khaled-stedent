# -*- coding: utf-8 -*-
"""
🚌 نظام إدارة نقل الطالبات الاحترافي
الإصدار الكامل: حافلات • سائقون • مسارات • طالبات • حضور • اشتراكات شهرية وحالة الدفع
التخزين: ملفات CSV دائمة في مجلد data/
التشغيل: streamlit run school_transport_pro.py
"""
import os
import pandas as pd
...
FILES = {
    ...
    "drivers":     ("drivers.csv",     ["السائق", "رقم التواصل", "الهوية/الرخصة", "اللوحة الم assigned", "تاريخ الإضافة"]),
    ...
    "payments":    ("payments.csv",    ["الشهر", "الطالبة", "المبلغ المطلوب", "المدفوع", "المتبقي", "حالة الدفع", "تاريخ آخر دفعة", "ملاحظات"]),
}
...
def load_df(key):
    ...
    return pd.read_csv(path, encoding="utf-8-sig") if os.path.exists(path) else pd.DataFrame(columns=FILES[key][1])
...
# ============================================================
#  إعدادات الصفحة والقائمة الجانبية
# ============================================================
...
if choice == "📊 لوحة التحكم":
    ...
    required = month_pay["المبلغ المطلوب"].sum() if not month_pay.empty else 0
    paid_amt = month_pay["المدفوع"].sum() if not month_pay.empty else 0
    ...
    row[2].metric("⏳ المتبقي", money(remaining))
    ...
    with right:
        ...
        if not month_pay.empty:
            ...
            if not late.empty:
                st.dataframe(late[["الطالبة", "المتبقي", "حالة الدفع"]], use_container_width=True, hide_index=True)
        else:
    ...
    with t2:
        if buses_df.empty:
            ...
            with st.form("edit_bus"):
                ...
                stt = st.selectbox("الحالة", ["جاهزة", "في الصيانة", "خارج الخدمة"],
                                   index=["جاهزة", "في الصيانة", "خارج الخدمة"].index(r["الحالة"]) if r["الحالة"] in ["جاهزة", "في الصيانة", "خارج الخدمة"] else 0)
...
# ============================================================
    ...
    with t1:
        with st.form("add_driver", clear_on_submit=True):
            ...
            plate = st.selectbox("الحافلة الم assigned", plates) if plates else st.text_input("رقم اللوحة")
    ...
    with t2:
        if drivers_df.empty:
            ...
            with st.form("edit_driver"):
                ...
                pl = st.selectbox("الحافلة", plates, index=plates.index(r["اللوحة الم assigned"]) if r["اللوحة الم assigned"] in plates else 0) if plates else st.text_input("اللوحة", value=str(r["اللوحة الم assigned"]))
                if st.form_submit_button("✏️ تحديث"):
                    drivers_df.loc[drivers_df["السائق"] == sel, ["رقم التواصل", "الهوية/الرخصة", "اللوحة الم assigned"]] = [ph, lc, pl]
                    save_df("drivers", drivers_df)
                    st.success("✅ تم التحديث.")
                    st.rerun()
...
# ============================================================
    ...
    with t2:
        if students_df.empty:
            ...
            with st.form("edit_student"):
                ...
                ro = st.selectbox("المسار", route_list, index=route_list.index(r["المسار"]) if r["المسار"] in route_list else 0) if route_list else st.text_input("المسار", value=str(r["المسار"]))
                pl = st.selectbox("الحافلة", plt_list, index=plt_list.index(r["اللوحة"]) if r["اللوحة"] in plt_list else 0) if plt_list else st.text_input("اللوحة", value=str(r["اللوحة"]))
            ...
            if upd:
                students_df.loc[students_df["الطالبة"] == sel, ["رقم التواصل", "اسم ولي الأمر", "الحي / موقع الاستلام", "المسار", "اللوحة", "الاشتراك الشهري", "الحالة"]] = [ph, par, ar, ro, pl, sb, stt]
                save_df("students", students_df)
                st.success("✅ تم التحديث.")
                st.rerun()
            if dele:
    ...
    with t3:
        if not students_df.empty:
            f = st.multiselect("تصفية حسب المسار", students_df["المسار"].unique().tolist())
            st.dataframe(students_df if not f else students_df[students_df["المسار"].isin(f)],
    ...
    if active.empty:
        ...
        for _, r in group.iterrows():
            ...
            if not existing.empty:
                ...
                if not m.empty:
                    d = ["حاضرة", "غائبة", "معتذرة"].index(m.iloc[0]["الحالة"])
            statuses[r["الطالبة"]] = st.radio(
                f"**{r['الطالبة']}** — {r['المسار']}",
                ["حاضرة", "غائبة", "معتذرة"], index=d, horizontal=True,
                key=f"att_{att_date}_{r['الطالبة']}")

        if st.button("💾 حفظ سجل الحضور"):
            att_df = att_df[att_df["التاريخ"] != str(att_date)] if not att_df.empty else att_df
...
# ============================================================
    ...
    t1, t2, t3 = st.tabs(["🧾 توليد فواتير الشهر", "💵 تسجيل دفعة", "📋 حالة الدفع"])
            ...
            if active.empty:
                ...
                for _, r in active.iterrows():
                    if r["الطالبة"] not in existing:
                        ...
                        pay_df.loc[len(pay_df)] = [month, r["الطالبة"], amt, 0, amt, "غير مدفوع", "", ""]
                        added += 1
    ...
    with t2:
        if pay_df.empty:
            ...
            if unpaid.empty:
                ...
                st.info(f"المطلوب: {money(r['المبلغ المطلوب'])} | المتبقي: {money(r['المتبقي'])}")
                amt = st.number_input("مبلغ الدفعة (ر.س)", min_value=0.0, value=float(r["المتبقي"]), step=10.0)
                note = st.text_input("ملاحظات", placeholder="نقدي / تحويل / جزئي...")
                if st.button("💵 تسجيل الدفعة"):
                    ...
                    save_df("payments", pay_df)
                    st.success(f"✅ تم تسجيل دفعة بقيمة {money(amt)} للطالبة {sel}.")
                    st.rerun()
    ...
    with t3:
        if pay_df.empty:
            ...
            c3.metric("📈 نسبة التحصيل", f"{round(view['المدفوع'].sum()/view['المبلغ المطلوب'].sum()*100,1) if view['المبلغ المطلوب'].sum() else 0}%")
...
# ============================================================
    ...
    with tab3:
        for key, label in [("buses", "الحافلات"), ("drivers", "السائقون"), ("routes", "المسارات"),
                           ("students", "الطالبات"), ("attendance", "الحضور"), ("payments", "الاشتراكات")]:
            df = load_df(key)
            if not df.empty:
