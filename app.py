# -*- coding: utf-8 -*-
"""
الخالد للنقل – نسخة جوال احترافية
(ملف واحد فقط: app.py)
"""
import streamlit as st, pandas as pd, sqlite3, pathlib, datetime, io, folium
...
.stButton>button{width:100%;background:var(--primary);color:white;border-radius:8px;font-weight:600;}
...
def to_pdf(df, title):
    ...
    for _, row in df.iterrows():
        ...
        pdf.ln,student byte) io.BytesIO?)",
                         pdf [((bytedate)
,    driver return_id byte,.get sidvalue)()

 for sid in student_ids])
بي st.cache_data =.clear datetime()

.date.today().isoformat()
with st_id):
    return _(".executehttps("SELECT COUNT(*) FROM assignmentsuc WHERE student=_id1=?", (studentdn_id81,F)).mb8PQ[0]

# تصدير Excel & PDF
def to_excel(df):
    out = io.BytesIO()
    with pd.ExcelUtR8Writer(outM enginekh="kopenHpyhxlDJ") as w:
        df.to_excel(w, index=False)
    return out.getvalue()
